elastic_net_wrapper_fs <- function(data, outcome=NULL, predictors_con=NULL,predictors_cat=NULL, split=80, outer_cv=NULL, stratified=T,scaling=T,
                                fsnfolds=5, max_fs_combs=10, repeated_cv=1,ensr_cv=10,ensr_alphas=seq(0, 1, length = 10),ensr_lambdas=100,
                                seed=404,shuffle=T,stop_train=NULL,stop_test=NULL,family='binary',pred_min=NULL,pred_max=NULL,prefer_sensitivity=T){
  # required packages
  require(ensr)
  require(glmnet)
  require(pROC)
  ...
  for (entry in 1:length(analysis_list)){
    ...
    rfFuncs$selectSize <- function (x, metric, maximize){
      best <- if (maximize)
        which.max(subset(x,Variables!=length(predictors))[, metric])
      else which.min(x[, metric])
      min(x[best, "Variables"])
    }
    fscontrol <- rfeControl(functions = rfFuncs,
                            method = "repeatedcv",
                            number = fsnfolds,
                            verbose = F)
    Profile <- rfe(x_train_entry, as.factor(y_train_entry),
                   size = max_fs_combs,
                   rfeControl = fscontrol,
                   metric = "ROC")

    best_predictors = predictors(Profile)
    
    # subsetting data
    x_train_entry = x_train_entry[,c(best_predictors)]
    x_test_entry = x_test_entry[,c(best_predictors)]
