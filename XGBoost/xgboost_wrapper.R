xgboost_wrapper <- function(data, outcome=NULL, predictors_con=NULL,predictors_cat=NULL, split=80, outer_cv=NULL, stratified=T,scaling=T,
                                 seed=404,shuffle=T,eta_range=c(0.001,0.4),ntrees_range = c(50L,200L),max_depth_range=c(6L,10L),subsample_range=c(0.8,1),
                            min_child_weight_range=c(1L,50L),colsample_bytree_range=c(0.8,1),lambda_range=c(1,1000),
  ...
  # create cluster
  if (exists('xgcluster')==F){
    if(is.null(ncores)==F){xgcluster<<-makeCluster(ncores)}
    else{
      ncores=detectCores()
      xgcluster<<-makeCluster(ncores)
      }
  }
  registerDoParallel(xgcluster)
  clusterEvalQ(xgcluster,library(xgboost))
  ...
  for (entry in 1:length(analysis_list)){
    ...
    scoringFunction <- function(eta,colsample_bytree,min_child_weight,max_depth,ntrees,subsample,lambda) {
    ...
    subsample = getBestPars(optObj)$subsample
     ...
     lambda=getBestPars(optObj)$lambda_range
    
    # fitting the xgboost model and getting the estimates for the variables
    set.seed(seed)
    xgmat = xgb.DMatrix(x_train_entry,label = y_train_entry)
    xgmodel = xgboost(data = xgmat
                      , nround = ntrees
                      , early_stopping_rounds = 5
                      , maximize = TRUE
                      , verbose = 0
    ...
    if (family=='binary'){
      # AUC
      predictions = predict(xgmodel, newdata=x_test_entry,type = "response")
    ...
    for (predictor in predictors){
      index = which(estimates[,1]==predictor)
      if (length(index)==0){
        results_df[entry,predictor]<- NA
      }
      else{
        results_df[entry,predictor]<- estimates[index,2]
      }
    }
    
    # Storing model
    models[[entry]]=xgmodel
