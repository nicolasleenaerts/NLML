elastic_net_wrapper_smote <- function(data, outcome=NULL, predictors_con=NULL,predictors_cat=NULL, split=80, outer_cv=NULL, stratified=T,scaling=T,
                                      repeated_cv=1,ensr_cv=10,ensr_alphas=seq(0, 1, length = 10),ensr_lambdas=100,seed=404,shuffle=T,
                                      stop_train=NULL,stop_test=NULL,dupSMOTE=1,KSMOTE=4){
  # required packages
  require(ensr)
  require(glmnet)
  require(pROC)
  ...
  for (entry in 1:length(analysis_list)){
    ...
    # correcting unbalanced data with smote
    set.seed(seed)
    data_smote = SMOTE(X = data.frame(x_train_entry),target=y_train_entry,dup_size = dupSMOTE,K=KSMOTE)
    y_train_entry = as.matrix(data_smote$data[,ncol(data_smote$data)])
    x_train_entry = as.matrix(data_smote$data[,-ncol(data_smote$data)])
    ...
    # Stopping if there aren't enough observations in the test data
