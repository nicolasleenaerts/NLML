null_model_pooled <- function(data, outcome=NULL, by=NULL,predictors_con=NULL,predictors_cat=NULL,
                                            between_predictors_con=NULL,between_predictors_cat=NULL,
                                            split=80, outer_cv=NULL,stratified=T,scaling=T,seed=404,
  ...
  for (entry in 1:length(analysis_list)){
    ...
    # storing estimates
    results_df_model[entry,predictors]<- NA
    ...
    for (by_index in 1:length(unique(unlist(data[by])))){
      ...
      if (family=='binary'){
        ...
        predictions = predictions = rep(0,length(y_test_by))
